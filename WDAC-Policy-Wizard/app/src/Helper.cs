...
namespace WDAC_Wizard
{
    ...
    public class VersionInfo
    {
        ...
        const String StringFileInfo = @"StringFileInfo\";
        const String OriginalFileNameStringName = @"\OriginalFileName";
        const String InternalNameStringName = @"\InternalName";
        const String FileDescriptionStringName = @"\FileDescription";
        const String ProductNameStringName = @"\ProductName";
        const String TranslationTable = @"\VarFileInfo\Translation";
        ...
        public static extern int GetFileVersionInfoSizeEx(uint dwFlags, [In] string lptstrFilename, out int lpdwHandle);

        [DllImport("version.dll", SetLastError = true, CharSet = CharSet.Unicode)]
        public static extern bool GetFileVersionInfoEx(uint dwFlags, [In] string lptstrFilename, int dwHandle, int dwLen, [Out] byte[] lpData);
        [DllImport("version.dll", SetLastError = true, CharSet = CharSet.Unicode)]
        public unsafe static extern bool VerQueryValue([In] void* pBlock, [In] string lpSubBlock, out void* lplpBuffer, out int puLen);

        // Build a dictionary where key=package install location, value= package family name
        static Dictionary<string, Tuple<string, string>> pfnMappingTable = null;
        static void BuildPFNDictionary()
        {
            if (pfnMappingTable != null)
            {
                return;
            }
            pfnMappingTable = new Dictionary<string, Tuple<string, string>>();
            PackageManager packageManager = new PackageManager();
            IEnumerable<Package> packages = (IEnumerable<Package>)packageManager.FindPackagesForUser(string.Empty);
            foreach (var p in packages)
            {
                try
                {
                    int major = (int)p.Id.Version.Major;
                    int minor = (int)p.Id.Version.Minor;
                    int build = (int)p.Id.Version.Build;
                    int revision = (int)p.Id.Version.Revision;

                    string stringVersion = major + "." + minor + "." + build + "." + revision;

                    pfnMappingTable.Add(p.InstalledLocation.Path, new Tuple<string, string>(p.Id.FamilyName, stringVersion));
                }
                catch (System.IO.FileNotFoundException)
                {
                    // Strangely this happens for HolographicViewer app on p.InstalledLocation
                }
            }
        }

        // input the full path to the file (including the filename) and this function will return
        // the PackageFamilyName (if any) that this file belongs to
        // NOTE: BuildPFNDictionary must be called before calling this function
        static Tuple<string, string> findPFNMatch(string fullFilePath)
        {
            // the algorithm is as follows: 
            //   (1)find the last "\" character in the input fullFilePath
            //   (2)see if there is a match to that substring path in the pfn dictionary
            //   (3)if yes, return the pfn
            //   (4)if not, find the next last "\"character in the input fullFilePath, and goto (2)
            int endIndex = fullFilePath.LastIndexOf("\\", StringComparison.OrdinalIgnoreCase);
            string testString = fullFilePath;
            while (testString != null && endIndex > 0)
            {
                testString = testString.Substring(0, endIndex);
                if (pfnMappingTable.ContainsKey(testString))
                {
                    return pfnMappingTable[testString];
                }
                endIndex = testString.LastIndexOf("\\", StringComparison.OrdinalIgnoreCase);
            }
            return null;
        }

        public unsafe VersionInfo(string filePath, string friendlyName)
        {
            this.Version = null;
            ...
            int viHandle;
            int verSize;
            if (0 == (verSize = GetFileVersionInfoSizeEx(FILE_VER_GET_NEUTRAL, fullPath, out viHandle)))
            {
                // unable to get version info
                return;
            }

            var buff = new byte[verSize];
            if (!GetFileVersionInfoEx(FILE_VER_GET_NEUTRAL, fullPath, viHandle, verSize, buff))
            {
                // unable to get version info
                return;
            }
            ...
            fixed (void* pBuff = buff)
            {
                ...
                {
                    // get 
                    if (!VerQueryValue(pBuff, @"\", out pRetBuff, out len))    // "\" refers to VS_FIXEDFILEINFO
                    {
                        throw Marshal.GetExceptionForHR(Marshal.GetHRForLastWin32Error());
                    }
                    var pFixedFileInfo = (VS_FIXEDFILEINFO*)pRetBuff;
                    fixedFileInfo = *pFixedFileInfo;	// copy

                    int major = (int)(fixedFileInfo.dwFileVersionMS >> 16);
                    int minor = (int)(fixedFileInfo.dwFileVersionMS & 0x0000ffff);
                    int build = (int)(fixedFileInfo.dwFileVersionLS >> 16);
                    int revision = (int)(fixedFileInfo.dwFileVersionLS & 0x0000ffff);
                ...
                int localeInfoLength = 0;
                void* localeInfoBlock = null;
                String localeInfoString = null;
                try
                {
                    // First get the first entry in the translation table
                    if (!VerQueryValue(pBuff, TranslationTable, out localeInfoBlock, out localeInfoLength))
                    {
                        throw Marshal.GetExceptionForHR(Marshal.GetHRForLastWin32Error());
                    }
                    short* pLocaleInfo = (short*)localeInfoBlock;

                    localeInfoString = pLocaleInfo[0].ToString("X4", CultureInfo.InvariantCulture) + pLocaleInfo[1].ToString("X4", CultureInfo.InvariantCulture);
                }
                catch (Exception)
                {
                    localeInfoString = null;
                }

                if (localeInfoString == null)
                {
                    this.OriginalFileName = null;
                    this.InternalName = null;
                    this.FileDescription = null;
                    this.ProductName = null;
                    this.PackageFamilyName = null;
                    this.PackageVersion = null;
                }
                else
                {
                    String PartialVerQueryPath = StringFileInfo + localeInfoString;
                    try
                    {
                        String FullOrigFileNamePath = PartialVerQueryPath + OriginalFileNameStringName;

                        if (!VerQueryValue(pBuff, FullOrigFileNamePath, out pRetBuff, out len))    // Pull default language OriginalFileName
                        {
                            throw Marshal.GetExceptionForHR(Marshal.GetHRForLastWin32Error());
                        }

                        this.OriginalFileName = Marshal.PtrToStringAuto((IntPtr)pRetBuff);
                    }
                    catch (Exception)
                    {
                        this.OriginalFileName = null;
                    ...
                    try
                    {
                        String FullInternalNamePath = PartialVerQueryPath + InternalNameStringName;

                        if (!VerQueryValue(pBuff, FullInternalNamePath, out pRetBuff, out len))    // Pull default language InternalName
                        {
                            throw Marshal.GetExceptionForHR(Marshal.GetHRForLastWin32Error());
                        }

                        this.InternalName = Marshal.PtrToStringAuto((IntPtr)pRetBuff);
                    }
                    catch (Exception)
                    {
                        this.InternalName = null;
                    ...
                    try
                    {
                        String FullFileDescriptionPath = PartialVerQueryPath + FileDescriptionStringName;

                        if (!VerQueryValue(pBuff, FullFileDescriptionPath, out pRetBuff, out len))    // Pull default language FileDescription
                        {
                            throw Marshal.GetExceptionForHR(Marshal.GetHRForLastWin32Error());
                        }

                        this.FileDescription = Marshal.PtrToStringAuto((IntPtr)pRetBuff);
                    }
                    catch (Exception)
                    {
                        this.FileDescription = null;
                    ...
                    try
                    {
                        String FullProductNamePath = PartialVerQueryPath + ProductNameStringName;

                        if (!VerQueryValue(pBuff, FullProductNamePath, out pRetBuff, out len))    // Pull default language ProductName
                        {
                            throw Marshal.GetExceptionForHR(Marshal.GetHRForLastWin32Error());
                        }

                        this.ProductName = Marshal.PtrToStringAuto((IntPtr)pRetBuff);
                    }
                    catch (Exception)
                    {
                        this.ProductName = null;
                    ...
                    BuildPFNDictionary();
                    Tuple<string, string> packageValue = findPFNMatch(friendlyName);
                    if (packageValue != null)
                    {
                        this.PackageFamilyName = packageValue.Item1;
                        this.PackageVersion = packageValue.Item2;
                    }
                    else
                    {
                        this.PackageFamilyName = null;
